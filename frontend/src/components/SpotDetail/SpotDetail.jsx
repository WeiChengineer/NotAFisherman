...
const SpotDetail = () => {
  ...
  useEffect(() => {
    ...
    if (!showDeleteModal || !showAddReviewModal) {
  ...
  const previewImage =
    spot.SpotImages?.find((image) => image.preview)?.url || defaultImage;
  ...
  const avgRatingDisplay =
    sumRating !== 0 ? (sumRating / reviewCount).toFixed(2) : "New";
  const beFirstPostReviewMessage =
    reviewCount === 0 && currentUser && currentUser.id !== spot.ownerId;

  const updateShowAddReviewModal = () => {
  ...
  const hasAlreadyReviewed = reviews.find((review) =>
    review.userId === currentUser.id ? true : false
  );
    ...
    <div className="">
          ...
          <div className="grid-container">
            ...
            {otherImages.slice(0, 4).map((url, index) => (
              <div className="spotCard" key={index}>
                <div className="card__image">
                  <img src={url} alt="spotCard Image" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="spotDetails">
            ...
            <div className="spotDetailsPrice">
              <div className="row justify-between items-center w-full">
                <h4 className="w-full">
                  ${parseFloat(spot.price).toFixed(2)} night
                </h4>
              ...
              <button
                className="w-full spotDetailsbtn"
          ...
          <section className="reviews">
            ...
            !hasAlreadyReviewed ? (
              <button
                ...
                onClick={updateShowAddReviewModal}
              >
                Post Your Review
              </button>
            ) : null}
      ...
      {showAddReviewModal ? (
        <AddReviewModal
          ...
          onClose={updateShowAddReviewModal}
        />
      ) : null}
      {showDeleteModal ? (
        <DeleteModal
